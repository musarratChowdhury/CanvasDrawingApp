import { GameCanvas } from "./CanvasDrawing.js";
const container = document.querySelector("#container");
const gameCanvas = document.querySelector("#game-canvas");
const display = document.querySelector(".count-display");
const colorSelection = document.querySelector("#color");
const linewidthSelection = document.querySelector("#lineWidth");
const clearBtn = document.querySelector("#clearButton");
let isDrawing = false;
let lastX = 0;
let lastY = 0;
var myGameCanvas = new GameCanvas(gameCanvas, window.innerWidth * 1, window.innerHeight * 0.7, "white");
...
gameCanvas.addEventListener("mousemove", (e) => {
    if (!isDrawing)
        return;
    // console.log(e);
    myGameCanvas.lineDrawing(lastX, lastY, e.offsetX, e.offsetY, Number(linewidthSelection.value), colorSelection.value);
    lastX = e.offsetX;
    lastY = e.offsetY;
});
